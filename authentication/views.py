...
class EmailValidationView(View):
    def post(self, request):
        ...
        if User.objects.filter(email=email).exists():
            return JsonResponse({'emailerror': 'Sorry, email is in use. Choose another one.'}, status=409)
...
class UsernameValidationView(View):
    def post(self, request):
        ...
        if User.objects.filter(username=username).exists():
            return JsonResponse({'usernameerror': 'Sorry, username is in use. Choose another one.'}, status=409)
            ...
            with transaction.atomic():
                if not User.objects.filter(username=username).exists():
                    if not User.objects.filter(email=email).exists():
                        ...
                        user.is_active = True  # Ensure the user can log in immediately
                        user.save()

                        # Send email for activation (if you want users to confirm their email)
